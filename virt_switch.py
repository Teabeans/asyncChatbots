...
# For timer signaling (Note: Does not work on Windows - signal.eggtimer() is only available in Unix)
# https://docs.python.org/3/library/signal.html
import signal

# For creating a timer eggtimer thread
...
#--------------------------------------|
    ...
    def timer_up(self):
        ...
        # End of timer instructions go here
...
#   #switch_terminate()
#--------------------------------------|
# Desc:    Deako-defined termination behavior incurred by MQTT termination callback
# Params:  NULL
# PreCons: NULL
# PosCons: NULL
# RetVal:  NULL
...
#          '<MeshID> <Command> <Value>'
# PosCons: NULL
# RetVal:  Dict - The value of the command, encapsulated as a string, an int, and a bool
# MetCall: NULL
        ...
        if (value == 'True' or value == 'true'):
            ...
            try:
                ...
                if(value == 'False'):
                    ...
                    retDict['int'] = 1
                elif(VERBOSE): print("Warning: Value ({}) is not castable as an int.".format(value))
    ...
    def respond(self, message, value):
        ...
        # All possible COMMANDS a switch can receive go here
            ...
            if (value['string'] == "!"):
                ...
                self._is_on = value['bool']

        # Wind up the timer
        elif (message == 'timer_duration'):
            if (VERBOSE): print("  ({}) Executing command: '{}' : ({})".format(self._mesh_id, message, value))
            self.eggtimer_duration = value['int']
            if (VERBOSE): print("  ({}) Eggtimer set: ({}) seconds".format(self._mesh_id, self.eggtimer_duration))

        # Attempt to start the timer
        elif (message == 'timer_start'):
            if (VERBOSE): print("  ({}) Executing command: '{}'".format(self._mesh_id, message))
            #
            # If this switch already has a timer somehow
            #
            if (self.eggtimer_exists == True):
                # But the timer is not running, yet is in a legal state to do so
                if (self.eggtimer_isPaused == True and self.eggtimer_duration > 0):
                    ...
                    # And note when we started it
                    self.eggtimer_startTime = datetime.datetime
                # Otherwise, the timer was not in a legal state to start...
                elif (self.eggtimer_isPaused == False or self.eggtimer_duration <= 0):
                    # So timer_start does nothing
                    if (VERBOSE): print("  ({}) Alarm not started. Paused: {}, Duration: {}".format(self._mesh_id, self.eggtimer_isPaused, self.eggtimer_duration))
            #
            # Otherwise, this switch does not yet have a timer
            #
            elif (self.eggtimer_exists == False):
                ...
                if (self.eggtimer_isPaused == True and self.eggtimer_duration > 0):
                    ...
                    self.eggtimer_startTime = datetime.datetime
                    self.eggtimer.start()
                    self.eggtimer_exists = True
                # But if the timer is already running or hasn't been wound up...
                elif (self.eggtimer_isPaused == False or self.eggtimer_duration <= 0):
                    # timer_start does nothing
                    if (VERBOSE): print("  ({}) Alarm not started. Paused: {}, Duration: {}".format(self._mesh_id, self.eggtimer_isPaused, self.eggtimer_duration))

        # Stops the timer by saving the time remaining and deleting the timer thread
        # Other timer commands will not function on a "paused" timer
        elif (message == 'timer_pause'):
            if (VERBOSE): print("  ({}) Executing command: '{}'".format(self._mesh_id, message))
            self.eggtimer_endTime = datetime.datetime
            ...
            if (VERBOSE): print("  ({}) Executing command: '{}' : ({})".format(self._mesh_id, message, value))
            # If the timer is paused... Warp just changes the remaining duration on the timer
            if (self.eggtimer_isPaused == True):
                self.eggtimer_duration = value['int']
            # But if the timer is running... Warp must cancel the previous thread, update the duration, and start again
            elif (self.eggtimer_isPaused == False):
                # Cancel the previous timer
                self.eggtimer.cancel()
            ...
            # Otherwise, Resume has no effect
                ...
                if (VERBOSE): print("  ({}) Timer already running, no action taken.".format(self._mesh_id))

        # Cancels a timer and sets the duration to 0
        elif (message == 'timer_cancel'):
            if (VERBOSE): print("  ({}) Executing command: '{}'".format(self._mesh_id, message))
            self.eggtimer_isPaused = True
            self.eggtimer.cancel()
...
def execute(command_array):
    ...
    for line in command_array:
        ...
        # All possible commands Control can SEND go in here
        # '0 report is_on'
        if (cmd == 'report'):
            if (val == 'is_on'):
        ...
        elif (cmd == 'timer_activate'):
            control.publish(TOPIC_QUERY, "{} {} {}".format(tgt, cmd, val))

        elif (cmd == 'timer_cancel'):
            control.publish(TOPIC_QUERY, "{} {} {}".format(tgt, cmd, val))

        elif (cmd == 'timer_duration'):
            control.publish(TOPIC_QUERY, "{} {} {}".format(tgt, cmd, val))

        elif (cmd == 'timer_pause'):
            control.publish(TOPIC_QUERY, "{} {} {}".format(tgt, cmd, val))

        elif (cmd == 'timer_resume'):
            control.publish(TOPIC_QUERY, "{} {} {}".format(tgt, cmd, val))
...
#            control.publish(TOPIC_QUERY, "{} {} {}".format(tgt)
        ...
        # Pause in instructions until user presses 'enter'
        input()
        
    print("(CTRL) Commands completed. Press 'enter' to begin termination...")
    print()
    print()
    input()
    ...
    if (value == 'True' or value == 'true'):
        ...
        try:
            ...
            if(value == 'False'):
                ...
                retDict['int'] = 1
            elif(VERBOSE): print("Warning: Value ({}) not castable as an int.".format(value))
